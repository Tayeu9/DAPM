...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['message'])) {
    ...
    $sql_get_maKH = "SELECT MaKH FROM khachhang WHERE TenTaiKhoan = '$username'";
    ...
    if (mysqli_num_rows($result_get_maKH) > 0) {
        ...
        $sql_insert_message = "INSERT INTO chat (MaKH, NoiDung, ThoiGian, MaNV) VALUES ('$maKH', '$message', NOW(), '$maNV')";
...
$sql_get_employees = "SELECT MaNV, TenNV FROM nhanvien";
...
<body>
    ...
    <h1>Chat</h1>
    <div class="welcome">Xin chào, <?php echo htmlspecialchars($username); ?> | <a href="logout.php"
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        ...
        <select name="receiver">
            ...
            while ($row = mysqli_fetch_assoc($result_get_employees)) {
                echo "<option value='" . htmlspecialchars($row['MaNV']) . "'>" . htmlspecialchars($row['TenNV']) . "</option>";
            }
            ?>
        </select>
    ...
    <ul>
        ...
        if (mysqli_num_rows($result_get_messages) > 0) {
            ...
            foreach ($messages_by_sender as $senderName => $messages) {
                echo "<h3>" . htmlspecialchars($senderName) . ":</h3>";
                echo "<ul>";
                foreach ($messages as $message) {
                    echo "<li>" . htmlspecialchars($message["NoiDung"]) . " - " . htmlspecialchars($message["ThoiGian"]) . "</li>";
                }
                echo "</ul>";
            }
        } else {
